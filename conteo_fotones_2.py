...
class Data:
    ...
    def hist(self,bines):
        plt.hist(self.l.voltaje, bins = bines, density = True, label = 'Con Laser', align = 'left',lw=1, ec="black")
        plt.hist(self.sl.voltaje,bins = bines, density = True, label = 'Sin Laser', align = 'left',lw=1, ec="black")
        plt.xlabel('Voltaje')
    ...
    def set_thres(self,threshold):
        ...
        if multiple == False:
            if self.ventana_tiempo == 'none':
                print('No se ha seteado una ventana temporal. Debe configurarlo segun .ventana_tiempo = Tiempo')
            else:
                self.conteo_por_pantalla = []
    ...
    def hist_conteo(self,bines):
            plt.hist(self.conteo_por_pantalla,bins = bines,lw=1, ec="black", label = 'Probabilidad de conteo', align = 'left')
    ...
    def tiempo_correlacion(self,pos):
        ...
        plt.xlabel('tiempo')
        plt.ylabel('Funcion de correlacion normalizada')
        plt.show()
